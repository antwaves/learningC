...
void _pick_physical_device(struct App* self) { // query physical devices and pick the best for vulkan to communicate with
...
static bool is_device_suitable(VkPhysicalDevice* device) { // checks if a physical device is capable of doing the things we need
    bool is_suitable;
    ...
    // check extension support for the device (swapchain and synchronization 2)
